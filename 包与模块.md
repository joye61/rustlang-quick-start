...
* **Package（包）**：一个由 `Cargo.toml` 管理的**项目单元**。包里可以包含**最多一个库 crate**，以及**任意多个二进制 crate**。
...
# 重新导出与对外 API 设计（`pub use` / Prelude）
